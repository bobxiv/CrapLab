...
//No se le pone extension al nombre
int Console_Icon::From_CHAR_to_BINARY( char* Name , char* Path ){
	...
	while( !input.eof() ){
		...
		//ya que los primeros 4 son el espacio reservado para Long
		tamano = output.tellp();
		tamano-=4;
...
int Console_Icon::Read_Console_Icon( char* Name , char* Path ){
	...
	//como Long me dan los elementos mas el fin de archivo, hay un lugar que no nesesito guardarlo
	if( Long == 0 ){
