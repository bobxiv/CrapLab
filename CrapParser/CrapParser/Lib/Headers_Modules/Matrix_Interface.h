...
public:
	...
	*/
	...
	//CONSTRUCTOR: Este es un constructor por copia, que utiliza un Vector. La matriz generada estara
	//en desarrollo de columnas, es decir sera un "vector fila"
	Matrix(const Vector<T> &V);
	
	//CONSTRUCTOR: Este constructor genera una matriz cuadrada de nxn, siendo n el parametro.
	//Tener en cuenta que la matriz tendra 1 o equivalente en la diagonal principal y
	...
	//Devuelve la matriz original, ie la matriz antes de ser gausada
	//
	...
	//Multiplica la matriz por un sierto valor que se pasa por parametro, ie multiplica cada
	...
	//Sobrecarga el operador * para una matriz por un escalar
	Matrix<T> operator *(const T );

	//Sobrecarga el operador * para un escalar or una matriz
	friend Matrix<T> operator*(const T esc, const Matrix<T> &B){
	...
	//Esta funcion realiza elevacion de poencia componente a componente
	//Las matrices deben ser de la misma dimension
	//Recibe:
	//-Una matriz que se elevara componente a componente con la matriz que llame a la funcion
	//Devuelve la matriz elevada
