///Clase templatizada, extiende RES_Base, representa un "mini arbol" que simboliza la operacion binaria de suma entre dos elementos
...
class Plus:public RES_Base<L,R,T,U>{
public:
	///Cosntructor basico
	///@param L hijo izquierdo
	///@param R hijo derecho
	///Llama al constructor identico del padre
	inline Plus( const L&, const R&);
	
	///Devuelve el elemento de la posicion especificada que estara en el resultado, luego de ejcutar la suma
	...
	///@param Store::Plus<L,R,T,U> el nuevo valor del objeto
	inline Store::Plus<L,R,T,U>& operator=( const Store::Plus<L,R,T,U>& );
...
///Especializacion de los templates Plus para la suma por un escalar, del lado IZQUIERDO
...
class Plus<L,R,L,U>:public RES_Base<L,R,L,U>{
public:
	///Cosntructor basico
	///@param L hijo izquierdo
	///@param R hijo derecho
	...
	///Devuelve el elemento de la posicion especificada que estara en el resultado, luego de ejcutar la suma
	///Notar que devuelve el tipo de dato mas "amplio", al que se le pasa typename para que el compilador 
	///sepa que es un tipo de dato, que por ser template sera constante en el tiempo de ejecucion
	///
	...
	///Operador de asignacion
	///@param Store::Plus<L,R,L,U> el nuevo valor del objeto
	inline Store::Plus<L,R,L,U>& operator=( const Store::Plus<L,R,L,U>& );

};


	template<class L, class R, class U>
inline typename promote<L,U>::res Store::Plus<L,R,L,U>::operator[](int i) const{
	...
	template<class L, class R, class U>
inline int Store::Plus<L,R,L,U>::Length() const{
	...
	template<class L, class R, class U>
inline Store::Plus<L,R,L,U>& Store::Plus<L,R,L,U>::operator=(const Store::Plus<L,R,L,U>& TWO ){
...
///Especializacion de los templates Plus para la suma por un escalar, del lado DERECHO
...
class Plus<L,R,T,R>:public RES_Base<L,R,T,R>{
public:
	///Cosntructor basico
	///@param L hijo izquierdo
	///@param R hijo derecho
	...
	///Devuelve la cantidad de componentes de el de la izquierdo
	///Primero se debe ver si es el mismo con la funcion:  Does_Components_Match()
	///Se llama asi, igual que el metodo de Vector, para compativilidad
	inline int Length() const;

	///Operador de asignacion
	///@param Store::Plus<L,R,T,R> el nuevo valor del objeto
	inline Store::Plus<L,R,T,R>& operator=( const Store::Plus<L,R,T,R>& );

};


	template<class L, class R, class T>
inline typename promote<T,R>::res Store::Plus<L,R,T,R>::operator[](int i) const{
	...
	template<class L, class R, class T>
inline int Store::Plus<L,R,T,R>::Length() const{
	...
	template<class L, class R, class T>
inline Store::Plus<L,R,T,R>& Store::Plus<L,R,T,R>::operator=(const Store::Plus<L,R,T,R>& TWO ){
...
//-----------------------------------  Definicion
	...
	template<class L, class R,class T,class U>
inline Plus<L,R,T,U>::Plus( const L& LEFT, const R& RIGHT):RES_Base<L,R,T,U>::RES_Base(LEFT,RIGHT){

}

	template<class L,class R,class T, class U>
inline typename promote<T,U>::res Plus<L,R,T,U>::operator[](int i) const{

	//Devuelve la componente resultado de la suma
	...
	template <class L,class R,class T,class U>
inline bool Plus<L,R,T,U>::Does_Components_Match(){
	...
	template <class L,class R,class T,class U>
inline int Plus<L,R,T,U>::Length() const{
	...
	template <class L,class R,class T,class U>
inline Store::Plus<L,R,T,U>& Store::Plus<L,R,T,U>::operator=( const Store::Plus<L,R,T,U>& TWO ){
