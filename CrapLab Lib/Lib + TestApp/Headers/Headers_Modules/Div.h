///Clase templatizada, extiende RES_Base, representa un "mini arbol" que simboliza la operacion binaria de division entre dos elementos
...
class Div:public RES_Base<L,R,T,U>{
public:
	...
	inline Store::Div<L,R,T,U>& operator=( const Store::Div<L,R,T,U>& );
...
///Especializacion de los templates Div para la division por un escalar, del lado IZQUIERDO
...
class Div<L,R,L,U>:public RES_Base<L,R,L,U>{
public:
	...
	///Devuelve el elemento de la posicion especificada que estara en el resultado, luego de ejcutar la division
	///Notar que devuelve el tipo de dato mas "amplio", yse le pasa typename para que el compilador 
	///sepa que es un tipo de dato, que por ser template sera constante en el tiempo de ejecucion
	///
	...
	inline Store::Div<L,R,L,U>& operator=( const Store::Div<L,R,L,U>& );
	...
	template<class L, class R, class U>
inline typename promote<L,U>::res Store::Div<L,R,L,U>::operator[](int i) const{

	//Devuelve la componente resultado de la division
	return ( Left / Right[i] );
	...
	template <class L, class R, class U>
inline int Store::Div<L,R,L,U>::Length() const{
	...
	template <class L,class R, class U>
inline Store::Div<L,R,L,U>& Store::Div<L,R,L,U>::operator=( const Store::Div<L,R,L,U>& TWO ){
...
///Especializacion de los templates Div para la division por un escalar, del lado DERECHO
...
class Div<L,R,T,R>:public RES_Base<L,R,T,R>{
public:
	...
	///Devuelve la cantidad de componentes de el de la izuqierda
	///Primero se debe ver si es el mismo con la funcion:  Does_Components_Match()
	///Se llama asi, igual que el metodo de Vector, para compativilidad
	inline int Length() const;
	...
	inline Store::Div<L,R,T,R>& operator=( const Store::Div<L,R,T,R>& );
	...
	template<class L, class R, class T>
inline typename promote<T,R>::res Store::Div<L,R,T,R>::operator[](int i) const{
	...
	template <class L, class R, class T>
inline int Store::Div<L,R,T,R>::Length() const{
	...
	template <class L,class R, class T>
inline Store::Div<L,R,T,R>& Store::Div<L,R,T,R>::operator=( const Store::Div<L,R,T,R>& TWO ){
...
//-----------------------------------  Definicion
	...
	template<class L, class R,class T,class U>
inline Div<L,R,T,U>::Div( const L& LEFT, const R& RIGHT):RES_Base<L,R,T,U>::RES_Base(LEFT,RIGHT){

}

	template<class L,class R,class T, class U>
inline typename promote<T,U>::res Div<L,R,T,U>::operator[](int i) const{
	...
	template <class L,class R,class T,class U>
inline bool Div<L,R,T,U>::Does_Components_Match(){
	...
	template <class L,class R,class T,class U>
inline int Div<L,R,T,U>::Length() const{
	...
	template <class L,class R,class T,class U>
inline Store::Div<L,R,T,U>& Store::Div<L,R,T,U>::operator=( const Store::Div<L,R,T,U>& TWO ){
