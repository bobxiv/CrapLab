		...
		inline T Matrix<T>::Get_Component( int I, int J ) const{
			...
			if( ( I <= this->F) && ( I > 0 ) && ( J <= this->C ) && ( J > 0 ) ){

			return ( this->MC[I-1][J-1] );

			}else{//si entra al else, es poruqe se va del rango, entonces devuelve este valor absurdo
	...
	*/
	...
	template <class T>
Matrix<T>::Matrix(){
	//este constructor no hace nada en especial
